...
import '../../../users/screens/shop/products/pizza_details.dart';
import 'add_pizza.dart';

class AdminPizza extends StatefulWidget {
  const AdminPizza({Key? key}) : super(key: key);

  @override
  State<AdminPizza> createState() => _AdminPizzaState();
}
...
class _AdminPizzaState extends State<AdminPizza> {
  ...
  Widget getData() {
    return StreamBuilder(
        stream: (Search.text == "")
            ?
        FirebaseFirestore.instance.collection("Products").doc("pizza").collection("Pizza").snapshots()
            :
        FirebaseFirestore.instance.collection('Products').doc("pizza").collection("Pizza").where("Product", arrayContains: Search.text).snapshots(),
  ...
  Widget MushroomsUi(String Name, String Id, String Image, String Price){
    return InkWell(
      onTap: (){
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => PizzaDetails(Id: Id,)),
        );
      },
      child: Card(
        elevation: 25.0,
        ...
        children: [
          SimpleDialogOption(
            ...
            onPressed: () async {
              ...
              FirebaseFirestore.instance.collection("Products").doc("pizza").collection("Pizza").doc(Id).delete();
